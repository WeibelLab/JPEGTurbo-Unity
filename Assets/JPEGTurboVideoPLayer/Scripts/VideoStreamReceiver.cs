...
namespace ARTEMIS.VideoStream
{
    public class VideoStreamReceiver : MonoBehaviour
    {
        ...
        #region Private info related to texxture
        ...
        [Tooltip("Causes quality to drop")]
        public bool BlazinglyFastDecode = false;
        ...
        public void OnEnable()
        {
            ...
            try
            {
                ...
                if (decoderHandle == IntPtr.Zero)
                {
                    ...
                    Debug.LogError(string.Format("[VideoStreamReceiver@{0}] Could not start LibJPEGTurbo (Internal Error)! Using Unity's built-in decoder (slow)...!", LogName));
                    decoderAvailabe = false;
                }
            } 
            catch (Exception e)
            {
                Debug.LogError(string.Format("[VideoStreamReceiver@{0}] Could not start LibJPEGTurbo! Using Unity's built-in decoder (slow)...!{1}", LogName, e));
                decoderAvailabe = false;
            }


            if (BlazinglyFastDecode)
            {
                decodingFlags = LibJPEGTurbo.TJFLAG_FASTDCT | LibJPEGTurbo.TJFLAG_BOTTOMUP;
            }
            else
            {
                decodingFlags = LibJPEGTurbo.TJFLAG_ACCURATEDCT | LibJPEGTurbo.TJFLAG_BOTTOMUP;
            }

            // we can run multi-threaded
            if (decoderAvailabe)
            {
                ...
                {
                    Debug.LogError(string.Format("[VideoStreamReceiver@{0}] Could not start LibJPEGTurbo Decoder Thread! Using Unity's built-in decoder (slow)...!{1}", LogName, e));
        ...
        public void OnDisable()
        {
            ...
            if (droppedFrames > 0)
            {
                Debug.LogWarning(string.Format("[VideoStreamReceiver@{0}] Dropped a total of {1} frames", LogName, droppedFrames));
            }
            ...
            Debug.LogWarning(string.Format("[VideoStreamReceiver@{0}] Stats: {1} frames displayed, {2} frames dropped, {3} frames decoded. Displayed at {4} fps. Decoded at {5} fps.", LogName, displayedFrames, droppedFrames, decodedFrames, displayedFrames/totalTimePlaying, decodedFrames/totalTimePlaying));
            ...
            if (decoderThreadRunning)
            {
                ...
                if (currentBuffer != null)
                {
                    ...
                    if (currentWidth != textureWidth || currentHeight != textureHeight)
                    {
                        ...
                        Debug.Log(string.Format("[VideoStreamReceiver@{0}] Allocated a texture buffer of {1}x{2}", LogName, textureWidth, textureHeight));
                    }
        ...
        {
            Debug.Log(string.Format("[VideoStreamReceiver@{0}] Started decoder thread", LogName));
            while (decoderThreadRunning)
            {
                try
                {
                    ...
                    while (decoderThreadRunning)
                    {
                        ...
                        while (decoderThreadRunning && tmpFrameQ.Count > 0)
                        {
                            ...
                            int width = 0x1FFFFFFF, height = 0x1FFFFFFF, jpegSubsample = 0x1FFFFFFF;

                            decodeError = false;
                            // decodes headers and allocates more memory if needed
                            if (LibJPEGTurbo.tjDecompressHeader2(decoderHandle, frame, frame.LongLength, ref width, ref height, ref jpegSubsample) == 0)
                            {
                                if (currentDecodeBuffer.buffer == null || width != currentDecodeBuffer.bufferWidth || height != currentDecodeBuffer.bufferHeight)
                                {
                                    ...
                                    // Debug.Log(string.Format("[VideoStreamReceiver@{0}] Allocated a buffer of {1}x{2}", LogName, width, height));


                                }

                                // decodes JPEG
                                if (LibJPEGTurbo.tjDecompress2(decoderHandle, frame, frame.LongLength, currentDecodeBuffer.buffer, currentDecodeBuffer.bufferWidth, 0, currentDecodeBuffer.bufferHeight, (int)LibJPEGTurbo.TJPF.TJPF_RGB, decodingFlags) == 0)
                                {
                            ...
                            if (decodeError)
                                ...
                                {
                                    Debug.LogError(string.Format("[VideoStreamReceiver@{0}] Unable to decode JPEG (Total decode errors so far {1})", LogName, decodedFrameErrors + 1));
                                }
                ...
                {
                    Debug.LogError(string.Format("[VideoStreamReceiver@{0}] Decoder thread threw an exception: {1}", LogName,e));
                }
            }
            Debug.Log(string.Format("[VideoStreamReceiver@{0}] Decoder thread exitted successfully", LogName));
